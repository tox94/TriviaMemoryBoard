package com.zavrsni.unizg.fer.triviamemoryboard.levels.first;
...
public class FirstLevelActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        board = (ScrollView) findViewById(R.id.scrollView_board);

        table = (TableLayout) findViewById(R.id.table_board);

        score = (TextView) findViewById(R.id.textView_score_points);

        fragQuestion = (View) findViewById(R.id.question_fragment);
        fragQuestion.setVisibility(View.INVISIBLE);
        fragQuestion.setEnabled(false);

        fragMove= (View) findViewById(R.id.last_moves_fragment);
        fragMove.setVisibility(View.INVISIBLE);
        fragMove.setEnabled(false);

        fragNick = findViewById(R.id.nick_fragment);
        ...
        fieldClicked = new Boolean[25];
        fieldClicked[0] = true;
        for(int field=1; field<25; field++){
    ...
    public void initTable(){
        ScrollView sv = (ScrollView) findViewById(R.id.scrollView_board);
        ...
        for(int i = 0; i<5; i++){
            ...
            params = new TableRow.LayoutParams(
                    svw/5-svw/50,
                    svh/5-svh/50);
            params.setMargins(svw/100, svw/100, svw/100, svw/100);
            for(int j=0; j<5; j++){
                if(i==4 && j==4){;
                    ...
                    iv.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {initLastMoveFragment(v);
                        }
                    });
                }else{
                        ...
                        public void onClick(View v) {
                            if(lastView.getId() != v.getId()){
                                if(firstPress == false){
                                    ...
                                    int[] fields;
                                    if(x%5 == 0)
                                        fields = new int[]{x-5, x+1, x+5};
                                    else if ((x+1)%5 == 0)
                                        fields = new int[]{x-5, x-1, x+5};
                                    else
                                        fields = new int[] {x-5, x-1, x+1, x+5};
        ...
        lastView = (ImageView) tr1.getChildAt(2);
    }

    public void executeMove(View v){

        if(v.getId() == lastView.getId()-5)
            chronoMoves.add("up");
        else if (v.getId() == lastView.getId()+5)
    ...
    public void checkAnswer(String ansx, String ansCorrect, int viewID){
        if(ansx.substring(3).equals(ansCorrect)){
            ...
            answer = true;
            corQ += 1;
            if(viewID == lastView.getId()-5)
                chronoMoves.add("up");
            else if (viewID == lastView.getId()+5)
                chronoMoves.add("down");
            else if (viewID == lastView.getId()-1)
                chronoMoves.add("left");
            else
                chronoMoves.add("right");
        }else{
    ...
    public void setClickableAndLocked(){
        ...
        setLc(new int[]{11, 16, 21}, new int[]{3, 8, 12, 13}, new int[]{18, 19, 23});
    ...
    public void changeClickable(int[] ints){
        ...
        try
        {
            for (int i : ints) {
                try {
                    if ((locked[i] == false || i == 21 || i == 12 || i == 23) && i >= 0 && i < 25) {
    ...
    public void changeLocked(int i){
        if(answer == true){
            if(i == 21){
                TableRow tr2 = (TableRow) table.getChildAt(2);
                TableRow tr3 = (TableRow) table.getChildAt(3);
                TableRow tr4 = (TableRow) table.getChildAt(4);
                ImageView iv1 = (ImageView) tr2.getChildAt(1);
                ImageView iv2 = (ImageView) tr3.getChildAt(1);
                ImageView iv3 = (ImageView) tr4.getChildAt(1);
                iv1.setBackgroundResource(R.drawable.field_button_style_red);
                iv2.setBackgroundResource(R.drawable.field_button_style_red);
                iv3.setBackgroundResource(R.drawable.field_button_style_red);
                ...
                int[] fields;
                if(i%5 == 0)
                    fields = new int[]{i-5, i+1, i+5};
                else if ((i+1)%5 == 0)
                    fields = new int[]{i-5, i-1, i+5};
                else
                    fields = new int[] {i-5, i-1, i+1, i+5};
                locked[11] = false;
                locked[16] = false;
                locked[21] = false;
                changeClickable(fields);
                lastView.setImageResource(0);
                lastView = iv3;
                iv3.setImageResource(R.mipmap.ic_my_location_black_36dp);
                ...
                TableRow tr0 = (TableRow) table.getChildAt(0);
                TableRow tr1 = (TableRow) table.getChildAt(1);
                TableRow tr2 = (TableRow) table.getChildAt(2);
                ImageView iv1 = (ImageView) tr0.getChildAt(3);
                ImageView iv2 = (ImageView) tr1.getChildAt(3);
                ImageView iv3 = (ImageView) tr2.getChildAt(2);
                ImageView iv4 = (ImageView) tr2.getChildAt(3);
                iv1.setBackgroundResource(R.drawable.field_button_style_red);
                iv2.setBackgroundResource(R.drawable.field_button_style_red);
                iv3.setBackgroundResource(R.drawable.field_button_style_red);
                iv4.setBackgroundResource(R.drawable.field_button_style_red);
                if(fieldClicked[i] == false) {
                    int n = Integer.parseInt(score.getText().toString());
                    n += 50;
                    score.setText("" + n);
                ...
                TableRow tr3 = (TableRow) table.getChildAt(3);
                TableRow tr4 = (TableRow) table.getChildAt(4);
                ImageView iv1 = (ImageView) tr3.getChildAt(3);
                ImageView iv2 = (ImageView) tr3.getChildAt(4);
                ImageView iv3 = (ImageView) tr4.getChildAt(3);
                iv1.setBackgroundResource(R.drawable.field_button_style_red);
                iv2.setBackgroundResource(R.drawable.field_button_style_red);
                iv3.setBackgroundResource(R.drawable.field_button_style_red);
                if(fieldClicked[i] == false) {
    ...
    public void endLevel(){
        ...
        btnEnd.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                int ord = 0;
                try{
                    ArrayList<ResultFromDb> allResults = rep.getAllResultsForUser(nick, 1);
                ...
                if(ord == 0)
                    ord = 1;
                ResultFromDb result = new ResultFromDb(cnt, nick, 1, seconds, points, corMoves, incMoves, ord, corQ, incQ);
